...
def my_grayscale(img):
    grey = np.zeros((img.shape[0], img.shape[1]))  # init 2D numpy array
    for rownum in range(len(img)):
        for colnum in range(len(img[rownum])):
            grey[rownum][colnum] = weightedAverage(img[rownum][colnum])/3
    return grey


## load an image
## convert it to grayscale 0.3*R + 0.6*G + 0.1*B
## apply average dithering with black and white
## display grayscale and result
## r + o